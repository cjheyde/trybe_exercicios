...
const STUDENT_ANSWERS = ["A", "C", "B", "D", "C", "N.A", "D", "A", "D", "C"];

const verificaRespostas = (RIGHT_ANSWERS, STUDENT_ANSWERS) => {
...
const contagemRespCorretas = (RIGHT_ANSWERS, STUDENT_ANSWERS, callback) => {
  let contador = 0;
  for (let index = 0; index < RIGHT_ANSWERS.length; index += 1) {
    const respostaCallback = callback(
      RIGHT_ANSWERS[index],
      STUDENT_ANSWERS[index]
    );
    contador += respostaCallback;
  }
  return `resultado final: ${contador} pontos`;
};

console.log(
  contagemRespCorretas(RIGHT_ANSWERS, STUDENT_ANSWERS, verificaRespostas)
);
